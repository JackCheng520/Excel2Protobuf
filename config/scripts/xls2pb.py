...
class SheetInterpreter:
    ...
    def __init__(self, xls_file, output_path, protoc):
        self._xls_file = xls_file
        self._output_path = output_path
        self._protoc = protoc

        try:
            self._workbook = xlrd.open_workbook(self._xls_file)
        except BaseException, e:
            print "open excel(%s) failed!"%(self._xls_file)
            raise
    ...
    def generate(self):
        ...
        for sheet in self._workbook.sheets():
            ...
            proto_name = str(sheet.cell_value(0, 0)).strip()
            if proto_name == "":
                print "invalid proto name sheet(%s)!"%(sheet.name)
                continue

            self._proto_name = proto_name
            self._pb_file_name = proto_name + ".proto"

            self._LayoutFileHeader()
            self._output.append("syntax = \"proto2\";\n")
            self._output.append("package Config;\n")
            self._LayoutStructHead(proto_name)
            ...
            try:
                os.system(self._protoc + " --python_out=" + self._output_path + " --proto_path=" + self._output_path + " " + self._pb_file_name)
            except BaseException, e:
                print "protoc failed!"
                raise

            exprot_class = str(sheet.cell_value(1, 0)).strip()
            if exprot_class == "":
                result["cs"].append(self._pb_file_name)
            elif exprot_class == "cs":
                result["cs"].append(self._pb_file_name)
    ...
    def _Write2File(self) :
        """输出到文件"""
        pb_file = open(self._output_path + self._pb_file_name, "w+")
...
class DataParser:
    """解析excel的数据"""
    def __init__(self, xls_file, output_path):
        self._xls_file = xls_file
        self._output_path = output_path
        try:
            self._workbook = xlrd.open_workbook(self._xls_file)
        except BaseException, e:
            print "open xls_file(%s) failed!"%(self._xls_file)
            raise
    ...
    def parse(self):
        for sheet in self._workbook.sheets():
            ...
            if proto_name == "":
                ...
                continue
            try:
                self._module_name = proto_name + "_pb2"
                sys.path.append(self._output_path)
            ...
            self._proto_name = proto_name
            item_array = getattr(self._module, proto_name + '_ARRAY')()

            # 先找到定义ID的列
            id_col = 0
            for id_col in range(0, self._col_count):
                if self._row == 0 | id_col == 0:
                    continue
    ...
    def _WriteData2File(self, data) :
        file = open(self._output_path + self._proto_name + ".bytes", 'wb+')
        file.write(data)
        file.close()

    def _WriteReadableData2File(self, data) :
        file = open(self._output_path + self._proto_name + ".txt", 'wb+')
        file.write(data)
        file.close()
